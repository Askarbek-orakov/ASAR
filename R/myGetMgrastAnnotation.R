...
#'@export
myGetMgrastAnnotation <-
  function(MetagenomeID,
           evalue = 5,
           identity = 60,
           length = 15,
    ...
    server.resource <-
      "http://api.metagenomics.anl.gov/1/annotation/similarity/"
...
mergeAnnots <- function(f, s) {
  ...
  names(d.merge) <-
    gsub('semicolon separated list of annotations.', '', names(d.merge))
  return(d.merge)
}
...
expandNamesDT <- function(x) {
  ...
  unique(d[, list(ufun = gsub('(\\]|\\[)', '', unlist(strsplit(fun, "\\]; *\\["))) ,
                  fun,
                  sp,
                  ab = .N) , by = md5sum]) -> d.ufun
  unique(d.ufun[, list(fun, usp = gsub('(\\]|\\[)', '', unlist(strsplit(sp, "\\]; *\\["))) , sp, ab) , by = .(md5sum, ufun)]) ->
    d.uspfun
  return(d.uspfun)
}
...
getAbundanceMD5FromDT <- function(d.ab) {
  d.ab <-
    d.ab[, .(sum = sum(ab), md5 = paste(md5sum, collapse = ',')), by = .(usp, ufun)]
...
getSpecieFromAbund <- function(file, sp, aggregate = FALSE) {
  ...
  if (aggregate &
      dim(d.res)[1] > 1)
    d.res <- aggregate(. ~ ufun, as.data.frame(d.res)[, -1], FUN = sum)
  return(d.res)
}
...
getFunctionFromAbund <- function(file, fun, aggregate = FALSE) {
  ...
  if (aggregate &
      dim(d.res)[1] > 1)
    d.res <- aggregate(. ~ usp, as.data.frame(d.res)[, -2], FUN = sum)
  return(d.res)
}
...
getFunctionFromAbundMD5 <- function(file, fun, aggregate = FALSE) {
  ...
  if (aggregate &
      dim(d.res)[1] > 1)
    d.res <- aggregate(. ~ usp, as.data.frame(d.res)[, -c(2, 3)], FUN = sum)
  return(d.res)
}
