...
#'@param MetagenomeID Id of your metagenome for which you get annotation from MG-RAST  as "mgm4736007.3"
...
getMgSample <- function(MetagenomeID, evalue = 5, identity = 60, length = 15, Webkey){
  kocomplete <- FALSE
  seedcomplete <- FALSE
  fseedcomplete <- FALSE
  while(kocomplete == FALSE){
    ko <- myGetMgrastAnnotation(MetagenomeID,resource = c(source = "KO", type = "ontology"), evalue = 5, identity = 60, length = 15, webkey = Webkey)
    if('Download complete' %in% tail(ko)){
      save(ko, file = paste0(MetagenomeID, ".ko"))
      kocomplete <- TRUE
      cat(paste0("ko file for sample ", MetagenomeID, "has been successfully downloaded!"))
    }
  }
  while(seedcomplete == FALSE){
    seed <- myGetMgrastAnnotation(MetagenomeID,resource = c(source = "SEED", type = "organism"), evalue = 5, identity = 60, length = 15, webkey = Webkey)
    if('Download complete' %in% tail(seed)){
      save(seed, file = paste0(MetagenomeID, ".seed"))
      seedcomplete <- TRUE
      cat(paste0("seed file for sample ", MetagenomeID, "has been successfully downloaded!"))
    }
  }
  while(fseedcomplete == FALSE){
    fseed <- myGetMgrastAnnotation(MetagenomeID,resource = c(source = "SEED", type = "function"), evalue = 5, identity = 60, length = 15, webkey = Webkey)
    if('Download complete' %in% tail(fseed)){
      save(fseed, file = paste0(MetagenomeID, ".fseed"))
      fseedcomplete <- TRUE
      cat(paste0("fseed file for sample ", MetagenomeID, "has been successfully downloaded!"))
    }
  }
  
}
myGetMgrastAnnotation("4736007.3" ,c("KO", "ontology"), "ibv5dnQkteDnpbsWU5YJj3JKH")
...
save(pwlist,d.bm,mdt,taxall,funtaxall, d.res,d.kres,fannot,sannot,ko,file = 'pathview.Rdata')
